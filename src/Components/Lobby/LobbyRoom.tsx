...
export default function LobbyRoom(props: any){
    ...
    useEffect(() => {
        ...
        const fetchLobby = async () => {
            ...
            const lobby = await client.records.getOne('lobbies', lobby_id, {'expand': 'players.user'});
                ...
                <div className="my-1 px-1 w-full overflow-hidden sm:w-1/4">
                     <h2 className="mb-2 text-lg font-semibold text-gray-900 dark:text-white">Players</h2>
                        <ul className="space-y-1 max-w-md list-inside text-gray-500 dark:text-gray-400">
                            {lobby['@expand'].players.map((player: any) => {
                                const user = player['@expand'].user;
                                const d = player.lobby_ready ? 'M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z' : 'M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z';
                                return (
                                <li key={user.id} className="flex items-center">
                                    <svg className={`w-4 h-4 mr-1.5 flex-shrink-0 ${player.lobby_ready ? 'text-green-500 dark:text-green-400' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d={d} clipRule="evenodd"></path></svg>
                                    {user.name}
                                </li>
                                );
                            })}
