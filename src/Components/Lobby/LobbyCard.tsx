...
export default function LobbyCard(props: any) {
    ...
    return (
        ...
        <div key={lobby.id} className="mt-3 p-6 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
            ...
            <button disabled={!lobby.waiting}  className={`inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white rounded-lg ${lobby.waiting ? 'focus:ring-4 hover:bg-blue-800 bg-blue-700 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800' : 'focus:outline-none bg-gray-700  dark:bg-gray-600  cursor-not-allowed'}`} onClick={() => {window.location.href= `/lobby/${lobby.id}`}}>
